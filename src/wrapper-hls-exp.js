class x2G_Player_HLS {
  HLS_JS = {
    ...
    // version: '1.2.9',
    // version: '1.3.5',
    // version: '1.4.14', // This code works for all hls.js versions up to v1.4.14.
    // version: '1.5.8',  // This code does not work for v1.5.0 and later versions of hls.js.
    get source() {return 'https://cdn.jsdelivr.net/npm/hls.js@' + this.version + '/dist/hls.js'}
  };
  ...
  check_HLS_JS() {
    this.script = document.getElementById('HLS_script_loading');
    if (this.script) {
      this.script.addEventListener('load', this.start.bind(this));
    } else if (document.getElementById('HLS_script_ready')) {
      this.start();
    } else {
      this.load_HLS_JS();
    }
  ...
  load_HLS_JS() {
    ...
    this.script.addEventListener('load', this.mark_HLS_JS.bind(this));
    this.script.src = this.HLS_JS.source;
    this.script.id = 'HLS_script_loading';
    document.head.appendChild(this.script);
  }

  mark_HLS_JS() {
    this.script.id = 'HLS_script_ready';
    this.start();
  }
